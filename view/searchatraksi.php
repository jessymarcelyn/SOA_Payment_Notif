        ...
        <div class="rounded-md w-[80vw] flex flex-col gap-5 bg-slate-50 p-5 shadow-md">
            ...
            <div class="w-full">
                <div class="inline-flex flex-col justify-center relative text-gray-500 w-full">
                    ...
                    <div class="relative">
                        <input type="text" name="attraction_id" id="attraction_id" class="w-full p-2 pl-10 rounded border border-gray-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent" placeholder="Attraction Name..."  />
            ...
            <div class="flex w-full relative gap-4">
                    ...
                    <div class="relative w-full mt-2">
                        ...
                        <select name="city" id="city" class="w-full p-2 pl-10 rounded border border-gray-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent">
                            <?php
                                    ...
                                    echo "<option value='$Pasuruan'>Prigen, Pasuruan</option>";
                                    echo "<option value='$Bali'>Jakarta</option>";
                                    echo "<option value='$Jakarta'>Bali</option>";
                            ?>
                        </select>
                ...
                <!-- Attraction Date -->
                <div class="w-1/3">
                    <label for="checkin" class="pb-3 text-gray-500">Date</label>
                    <div class="relative w-full mt-2">
                        <input type="date" name="checkin" id="checkin" class="w-full p-2 pl-10 rounded border border-gray-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent" placeholder="search..." />
...
<script>
    $(document).ready(function(){
        ...
        $('#attraction_id').on('input', function(){
            var search = $(this).val();
            if(search != ''){
                // $.ajax({
                //     url: 'search.php',
                ...
                //     success: function(response){
                //         $('#cityList').html(response);
                        $('#attractionList').removeClass('hidden')
                        console.log('searching...');
                //     }
                // });
            }
        ...
        $(document).on('click', 'li', function(){
            $('#attraction_id').val($(this).text());
           
            // remove space
            $('#attraction_id').val($('#attraction_id').val().replace(/\s/g, ''));
            $('#attractionList').addClass('hidden');
